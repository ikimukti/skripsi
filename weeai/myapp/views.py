...
menus = [
    ...
    },
    {
        ...
        "submenus": [
            {
                "name": "Segmentation",
                "url": "/report/segmentation/",
                "icon": "fas fa-chart-pie",
                "id": "reportSegmentation",
            },
            {
                "name": "Export Image",
                "url": "/report/export/image/",
                "icon": "fas fa-file-image",
                "id": "reportExportImage",
            },
            {
                "name": "Export Report",
                "url": "/report/export/report/",
                "icon": "fas fa-file-pdf",
                "id": "reportExportReport",
            },
            {
                "name": "Summary",
                "url": "/report/summary/",
...
def calculate_scores(
    ...
    if (
        scores["precision"] == "nan"
        or scores["precision"] == "inf"
        or scores["precision"] == "-inf"
        or scores["precision"] == "None"
        or scores["precision"] == "0.0"
        or scores["precision"] == "0"
    ):
        # reverse segmented value
        reverse_segmented = np.where(segmented == 0, 1, 0)
        scores["precision"] = str(
    ...
    if (
        scores["recall"] == "nan"
        or scores["recall"] == "inf"
        or scores["recall"] == "-inf"
        or scores["recall"] == "None"
        or scores["recall"] == "0.0"
        or scores["recall"] == "0"
    ):
        # reverse segmented value
        reverse_segmented = np.where(segmented == 0, 1, 0)
    ...
    scores["accuracy"] = str(
        round(accuracy_score(ground_truth.flatten(), segmented.flatten()), 4)
    )
    if (
        scores["accuracy"] == "nan"
        or scores["accuracy"] == "inf"
        or scores["accuracy"] == "-inf"
        or scores["accuracy"] == "None"
        or scores["accuracy"] == "0.0"
        or scores["accuracy"] == "0"
    ):
        # reverse segmented value
        reverse_segmented = np.where(segmented == 0, 1, 0)
        scores["accuracy"] = str(
            round(
                accuracy_score(ground_truth.flatten(), reverse_segmented.flatten()), 4
            )
        )
    scores["rand"] = str(
        round(rand_score(ground_truth.flatten(), segmented.flatten()), 4)
    )
    if (
        scores["rand"] == "nan"
        or scores["rand"] == "inf"
        ...
        reverse_segmented = np.where(segmented == 0, 1, 0)
        scores["rand"] = str(
            round(rand_score(ground_truth.flatten(), reverse_segmented.flatten()), 4)
        )
    scores["jaccard"] = str(
        round(
            jaccard_score(
    ...
    if (
        scores["jaccard"] == "nan"
        or scores["jaccard"] == "inf"
        or scores["jaccard"] == "-inf"
        or scores["jaccard"] == "None"
        or scores["jaccard"] == "0.0"
        or scores["jaccard"] == "0"
    ):
        # reverse segmented value
        reverse_segmented = np.where(segmented == 0, 1, 0)
    ...
    scores["mae"] = str(
        round(mean_absolute_error(ground_truth.flatten(), segmented.flatten()), 4)
    )
    scores["rmse"] = str(
        round(
            mean_squared_error(
                ground_truth.flatten(), segmented.flatten(), squared=False
            ),
            4,
        )
    )
...
class ImageClassView(View):
    ...
    def get(self, request):
        ...
        xsegmentation_results = XSegmentationResult.objects.filter(
            idImage=OuterRef("pk")
        )
        ...
        xsegmentation_result_fields = {
            f"xsegmentation_result_{field}": Subquery(
                xsegmentation_results.values(field)[:1]
            )
            for field in fields
        }
...
class ImageSummarySingleClassView(View):
    ...
    def generate_chart(self, title, chartName, y_labels, unit):
        data = self.context["imageSingle"]["xsegmentation_result_report"][
            "kmeans_scores"
        ]

        # x adalah kombinasi dari 'scaleRatio', 'contrastEnhancement' 'GroundTruthKe'
        x = [
            f"{item['scaleRatio']},{item['contrastEnhancement']},{item['GroundTruthKe']}"
            for item in data
        ]
        ...
        x = [
            f"{item['scaleRatio']/10}:1 , {item['contrastEnhancement']/10}:1 , {item['GroundTruthKe']}"
            for item in data
        ]
        ...
        if unit == "%":
            # y_values * 100
            y_values = [
                [float(item[label]) * 100 for item in data] for label in y_labels
            ]
        else:
    ...
    def get(self, request, id):
        ...
        self.generate_chart(
            "Akurasi Adaptive Thresholding", "chartAdaptive", y_labels, unit="dB"
        )
        y_labels = ["f1", "precision", "recall", "accuracy", "rand", "jaccard"]
        self.generate_chart("Akurasi K-Means", "chartKmeans2", y_labels, unit="%")
        self.generate_chart(
            "Akurasi Adaptive Thresholding", "chartAdaptive2", y_labels, unit="%"
        )
...
class ImageUploaderClassView(View):
    ...
    def get(self, request, uploader):
        ...
        if "q" in request.GET:
            ...
            # Mendapatkan semua objek XImage
            ximages = (
                XImage.objects.filter(uploader=uploader)
                .order_by("-dateModified")
                .distinct()
            )
...
def kmeans_segmentation_scores(
    image_path,
    k=2,
    scaleRatio=1.0,
    contrastEnhancement=1.0,
    imgGroundTruth=None,
    imgGroundTruth2=None,
):
    img = image_path
    ...
    for i in range(int(beforeScaleRatio), int(afterScaleRatio), 1):
        # Resize the image
        imgScale = cv.resize(
            img, (int(img.shape[1] * i / 10), int(img.shape[0] * i / 10))
        )
        print("imgScale: ", imgScale.shape)
        imgGT = cv.resize(
            imgGroundTruth,
            (
                int(imgGroundTruth.shape[1] * i / 10),
                int(imgGroundTruth.shape[0] * i / 10),
            ),
        )
        ...
        imgGT2 = cv.resize(
            imgGroundTruth2,
            (
                int(imgGroundTruth2.shape[1] * i / 10),
                int(imgGroundTruth2.shape[0] * i / 10),
            ),
        )
        ...
        for j in range(
            int(beforeContrastEnhancement), int(afterContrastEnhancement), 1
        ):
            print("Contrast Enhancement: ", j / 10)
            # Image contrastEnhancement with method 1
            contrastEnhancementimg = cv.addWeighted(
                imgScale, j / 10, np.zeros(imgScale.shape, imgScale.dtype), 0, 0
            )
            ...
            img_2d2 = img_2d2.reshape(
                img_2d2.shape[0] * img_2d2.shape[1], img_2d2.shape[2]
            )
            ...
            ret, labels, centers = cv.kmeans(
                img_2d, k, None, criteria, 10, cv.KMEANS_RANDOM_CENTERS
            )
            ret2, labels2, centers2 = cv.kmeans(
                img_2d2, k, None, criteria2, 10, cv.KMEANS_RANDOM_CENTERS
            )
...
def adaptive_threshold_segmentation(image_path, block_size, c):
    ...
    segmented_img = cv.adaptiveThreshold(
...
def adaptive_threshold_segmentation_scores(
    image_path,
    block_size,
    c,
    scaleRatio=1.0,
    ...
    for i in range(int(beforeScaleRatio), int(afterScaleRatio), 1):
        ...
        for j in range(
            ...
            gray_img = cv.cvtColor(imgScale, cv.COLOR_BGR2GRAY)
            contrastEnhancementimg = cv.cvtColor(
                contrastEnhancementimg, cv.COLOR_BGR2GRAY
            )
            # Image contrastEnhancement with method 2
            gray_img = cv.convertScaleAbs(gray_img, alpha=j / 10, beta=0)
            ...
            segmented_img2 = cv.adaptiveThreshold(
                contrastEnhancementimg2,
                255,
                cv.ADAPTIVE_THRESH_MEAN_C,
                cv.THRESH_BINARY,
                block_size,
...
class ImageUploadClassView(View):
    ...
    def post(self, request):
        ...
        existing_files = [
            f.name for f in os.scandir("myapp/static/myapp/images/") if f.is_file()
        ]
        ...
        for file in files:
            ...
            imgDominantColor = imgDominantColor.reshape(
                (imgDominantColor.shape[0] * imgDominantColor.shape[1], 3)
            ).astype(np.float32)
            ...
            _, labels, centers = cv.kmeans(
                imgDominantColor, 3, None, criteria, 10, flags
            )
            ...
            imgScale = cv.resize(
                img, (int(img.shape[1] * scaleRatio), int(img.shape[0] * scaleRatio))
            )
            ...
            contrastEnhancementimg = cv.addWeighted(
                img, contrastEnhancement, np.zeros(img.shape, img.dtype), 0, 0
            )
            cv.imwrite(
                "myapp/static/myapp/images/" + name + "_enhanced.jpg",
                contrastEnhancementimg,
            )
            ...
            cv.imwrite(
                "myapp/static/myapp/images/" + name + "_enhanced2.jpg",
                contrastEnhancement2img,
            )
            pathContrastEnhancement2 = name + "_enhanced2.jpg"
            ...
            if noiseReduction == True:
                ...
                cv.imwrite(
                    "myapp/static/myapp/images/" + name + "_noiseReduction.jpg", imgnR
                )
                pathNoiseReduction = name + "_noiseReduction.jpg"
                cv.imwrite(
                    "myapp/static/myapp/images/" + name + "_noiseReduction2.jpg", imgnR2
                )
            ...
            XImage.objects.create(
                ...
                sizeImage={
                    ...
                    "channel": channels,
                    # dalam satuan mb
                    "size": os.path.getsize("myapp/static/myapp/images/" + name)
                    / 1024
                    / 1024,
                },
            )
            ...
            cv.imwrite(
                "myapp/static/myapp/images/" + name + "_ground_truth.jpg", ground_truth
            )
            pathGroundTruth = name + "_ground_truth.jpg"
            cv.imwrite(
                "myapp/static/myapp/images/" + name + "_ground_truth2.jpg",
                ground_truth2,
            )
            ...
            kmeans_scores = kmeans_segmentation_scores(
                img,
                2,
                scaleRatio,
                contrastEnhancement,
                ground_truth_noScale,
                ground_truth_noScale2,
            )
            adaptive_scores = adaptive_threshold_segmentation_scores(
                img,
                11,
                2,
                scaleRatio,
            ...
            cv.imwrite(
                "myapp/static/myapp/images/" + name + "_segKMeans.jpg", segKMeans
            )
            pathKMeans = name + "_segKMeans.jpg"
            segKMeans2 = kmeans_segmentation(imgnR2, 2)
            cv.imwrite(
                "myapp/static/myapp/images/" + name + "_segKMeans2.jpg", segKMeans2
            )
            ...
            cv.imwrite(
                "myapp/static/myapp/images/" + name + "_segAdaptive.jpg", segAdaptive
            )
            pathAdaptive = name + "_segAdaptive.jpg"
            segAdaptive2 = adaptive_threshold_segmentation(imgnR2, 11, 2)
            cv.imwrite(
                "myapp/static/myapp/images/" + name + "_segAdaptive2.jpg", segAdaptive2
            )
            ...
            cv.imwrite(
                "myapp/static/myapp/images/" + name + "_segCanny2.jpg", segCanny2
            )
            ...
            cv.imwrite(
                "myapp/static/myapp/images/" + name + "_segSobel2.jpg", segSobel2
            )
            ...
            cv.imwrite(
                "myapp/static/myapp/images/" + name + "_segPrewitt.jpg", segPrewitt
            )
            pathPrewitt = name + "_segPrewitt.jpg"
            cv.imwrite(
                "myapp/static/myapp/images/" + name + "_segPrewitt2.jpg", segPrewitt2
            )
            ...
            scores_sobel2 = calculate_scores(
                ground_truth2, segSobel2, "sobel", "weighted"
            )
            # print(scores_sobel2)
